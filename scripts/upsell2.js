    ...
    const response = await fetch(
      `https://app-cms-api-proxy-prod-001.azurewebsites.net/vrio/order_offers`,
      {
        ...
        body: JSON.stringify({
          ...
          order_id: lastOrderId,
          pageId: "pCVvXFmqmnPmPG4ZxNRAT_7kTnalo1PBo3mhSdYQPVjJSFwsOYLi3JHUSGm-n7z1"
        })
      }
    );
...
const processUpsell = async () => {
  ...
  try {
    const orderData = JSON.parse(sessionStorage.getItem("orderData"));
    orderData.pageId = "pCVvXFmqmnPmPG4ZxNRAT_7kTnalo1PBo3mhSdYQPVjJSFwsOYLi3JHUSGm-n7z1";
    const lastOrderId = sessionStorage.getItem("cms_oid");
